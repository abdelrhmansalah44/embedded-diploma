...
typedef struct{
	...
	uint16_t trigger_case;  					 // Specifies RISING or FALLING or both trigger.
	                                             // This parameter must be set based on @ref EXTI_TRIGGER_define


	uint16_t IRQ_enable; 						// Enable or Disable the EXTI IRQ (That will enable the IRQ mask in EXTI and NVIC controller)
...
#define EXTI_PIN_ENABLE			1
#define EXTI_PIN_DISABLE		0

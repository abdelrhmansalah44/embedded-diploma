...
UART_config *Global_UARTx_Cnfg[3] = {NULL,NULL,NULL};
...
void MCAL_GPIO_uart_init(UART_typedef * UARTx)
{
	...
	if(Global_UARTx_Cnfg[0]->flow_ctrl == uart_flowctrl_RTS || Global_UARTx_Cnfg[0]->flow_ctrl == uart_flowctrl_CTS_RTS)
	{
	//rts
	...
	else if(Global_UARTx_Cnfg[0]->flow_ctrl == uart_flowctrl_CTS || Global_UARTx_Cnfg[0]->flow_ctrl == uart_flowctrl_CTS_RTS)
	{
	//cts
	...
	MCAL_GPIO_INIT(GPIOA, &gpio_config);
		...
		if (Global_UARTx_Cnfg[1]->flow_ctrl == uart_flowctrl_RTS || Global_UARTx_Cnfg[1]->flow_ctrl == uart_flowctrl_CTS_RTS) {
			//rts
		...
		else if (Global_UARTx_Cnfg[1]->flow_ctrl == uart_flowctrl_CTS || Global_UARTx_Cnfg[1]->flow_ctrl == uart_flowctrl_CTS_RTS) {
			//cts
	...
	//uart3   pb10 tx, pb11 rx ,pb13 cts, pb14 rts
		...
		if (Global_UARTx_Cnfg[2]->flow_ctrl == uart_flowctrl_RTS || Global_UARTx_Cnfg[2]->flow_ctrl == uart_flowctrl_CTS_RTS) {
			//rts
		...
		else if (Global_UARTx_Cnfg[2]->flow_ctrl == uart_flowctrl_CTS || Global_UARTx_Cnfg[2]->flow_ctrl == uart_flowctrl_CTS_RTS) {
			//cts
